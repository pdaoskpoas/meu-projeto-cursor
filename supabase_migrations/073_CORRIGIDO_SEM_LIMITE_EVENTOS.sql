...
-- VERSÃO CORRIGIDA: SEM LIMITE DE EVENTOS ATIVOS
-- ========================================
-- Data: 24/11/2025
-- Descrição: Remove limite de eventos ativos. Usuário pode ter quantos quiser (pagando individual).
-- ========================================
...
-- Step 6: Function to check if user can create event (CORRIGIDA)
...
BEGIN
  ...
  -- Calcular cota e disponibilidade
  ...
  -- Contar eventos ativos (apenas para informação)
  ...
  -- ❌ REMOVIDO: Verificação de limite de 1 evento ativo
  -- Usuário pode ter quantos eventos ativos quiser!

  -- Caso 1: Plano inválido ou Free
  ...
  -- Caso 2: Plano sem cota de eventos (Basic, VIP)
  ...
  -- Caso 3: Cota mensal esgotada
  IF v_publications_used >= v_publications_quota THEN
    RETURN jsonb_build_object(
      'can_create', false,
      'reason', 'monthly_quota_exhausted',
      'message', format(
        'You have used %s of %s publication(s) this month. Next reset: %s. You can still publish by paying R$ 49,99 individually or upgrading your plan.',
        v_publications_used,
  ...
  -- Caso 4: Pode publicar com cota do plano! ✅
  RETURN jsonb_build_object(
    'can_create', true,
    'reason', 'within_quota',
    'message', format(
      'You can publish this event with your plan quota. %s publication(s) remaining this month.',
...
BEGIN
  ...
  -- 1. Evento está sendo ativado (ad_status = 'active')
  -- 2. NÃO é publicação individual (is_individual_paid = false ou null)
  -- 3. É INSERT ou UPDATE de draft para active
...
-- Step 9: Function to process individual payment
...
BEGIN
  ...
  -- Ativar evento como publicação individual
  UPDATE events
  SET 
...
-- Step 10: Update existing active events
UPDATE events
...
-- Step 11: Update older events (already past edit window)
UPDATE events
SET can_edit_until = published_at + INTERVAL '24 hours'
WHERE ad_status = 'active' 
  AND published_at IS NOT NULL
...
-- ========================================
-- INSTRUÇÕES PARA CRON JOB (MANUAL)
-- ========================================
-- Executar no Supabase Dashboard > Database > Cron Jobs
-- 
...
DO $$ 
BEGIN 
  RAISE NOTICE '✅ Migration 073 CORRIGIDA aplicada com sucesso!';
  RAISE NOTICE '📊 Usuários podem ter QUANTOS eventos ativos quiserem';
  RAISE NOTICE '💰 Limite é apenas nas publicações GRATUITAS do plano';
  RAISE NOTICE '🎯 Publicações individuais (R$ 49,99) não têm limite';
END $$;
