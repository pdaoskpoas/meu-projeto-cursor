...
-- Ref: RELATORIO_AUDITORIA_SOCIEDADES_PROFISSIONAL_2025-11-17.md
-- =====================================================

BEGIN;

RAISE NOTICE '🚀 Iniciando Migration 065: Sistema de Código Exclusivo por Animal';

-- =====================================================
-- PASSO 1: ADICIONAR CAMPO SHARE_CODE NA TABELA ANIMALS
-- =====================================================

RAISE NOTICE '  ➜ Passo 1: Adicionando campo share_code...';
...
RAISE NOTICE '  ✓ Campo share_code adicionado com sucesso';

-- =====================================================
-- PASSO 2: FUNÇÃO PARA GERAR CÓDIGO ÚNICO
-- =====================================================

RAISE NOTICE '  ➜ Passo 2: Criando função para gerar códigos únicos...';
...
BEGIN
    LOOP
        ...
        -- ANI = Prefix Animal
        -- XXXXXX = 6 caracteres alfanuméricos maiúsculos
        -- YY = Ano (25 para 2025)
        random_code := 'ANI-' || 
...
RAISE NOTICE '  ✓ Função generate_animal_share_code criada';

-- =====================================================
-- PASSO 3: TRIGGER PARA GERAR CÓDIGO AUTOMATICAMENTE
-- =====================================================

RAISE NOTICE '  ➜ Passo 3: Configurando trigger automático...';
...
RAISE NOTICE '  ➜ Passo 4: Populando códigos para animais existentes...';
...
BEGIN
  ...
  RAISE NOTICE '    • Encontrados % animais sem código', total_animals;
  ...
  RAISE NOTICE '    ✓ Total de % códigos gerados', processed_count;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '  ✓ Validação: % animais, todos com código único', total_animals;
END $$;
...
RAISE NOTICE '  ➜ Passo 5: Migrando dados de sociedades...';
...
-- Popular added_by com partner_id para registros existentes (auto-adicionado)
UPDATE public.animal_partnerships 
SET added_by = partner_id 
WHERE added_by IS NULL;

RAISE NOTICE '    • Novas colunas adicionadas e populadas';
...
BEGIN
  ...
  -- AÇÃO: Deletar convites não aceitos
  -- IMPORTANTE: Avisar usuários antes em produção!
  IF count_pending > 0 OR count_rejected > 0 THEN
    RAISE WARNING '    ⚠️  Deletando % convites pendentes/rejeitados', (count_pending + count_rejected);
...
RAISE NOTICE '    • Colunas obsoletas removidas (status, partner_public_code)';
...
RAISE NOTICE '  ➜ Passo 6: Atualizando funções SQL...';
...
DECLARE
    total_count INTEGER;
BEGIN
    -- Contar animais próprios ativos + sociedades ativas
    ...
    FROM (
        ...
        -- Animais em sociedade (sem filtro de status - agora todos são aceitos)
        SELECT ap.animal_id
...
-- 6.2: Atualizar função should_animal_be_active (remover filtro de status)
...
-- 6.3: Atualizar função get_profile_animals (remover filtro de status)
...
BEGIN
    ...
    LEFT JOIN public.animal_partnerships ap 
        ...
        -- Removido filtro: AND ap.status = 'accepted' (não existe mais)
    LEFT JOIN (
...
RAISE NOTICE '    ✓ Funções SQL atualizadas (3 modificadas, 1 removida)';

-- =====================================================
-- PASSO 7: ATUALIZAR VIEW ANIMALS_WITH_PARTNERSHIPS
-- =====================================================

RAISE NOTICE '  ➜ Passo 7: Atualizando view animals_with_partnerships...';
...
SELECT 
    ...
    -- Array de sócios (sem filtro de status - todos são aceitos agora)
    COALESCE(
        json_agg(
            json_build_object(
...
RAISE NOTICE '  ➜ Passo 8: Atualizando sistema de notificações...';
...
RAISE NOTICE '  ✓ Sistema de notificações atualizado';

-- =====================================================
-- PASSO 9: ATUALIZAR RLS POLICIES
-- =====================================================

RAISE NOTICE '  ➜ Passo 9: Atualizando políticas de segurança (RLS)...';

-- Atualizar policy de visualização para sócios (remover filtro de status)
...
RAISE NOTICE '  ➜ Passo 10: Configurando permissões...';
...
-- =====================================================
-- PASSO 11: VALIDAÇÕES FINAIS
-- =====================================================

RAISE NOTICE '  ➜ Passo 11: Executando validações finais...';
...
BEGIN
  ...
  RAISE NOTICE '    • Campo share_code adicionado à tabela animals';
  RAISE NOTICE '    • Trigger automático de geração de código criado';
  ...
  RAISE NOTICE '    1. Atualizar Service Layer (partnershipService.ts)';
  RAISE NOTICE '    2. Refatorar Frontend (SocietyPage, AnimalPage)';
  RAISE NOTICE '    3. Executar testes E2E';
  RAISE NOTICE '    4. Atualizar documentação';
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════';
  RAISE NOTICE '';
END $$;
